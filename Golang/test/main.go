...
func errorMessage_min(fn interface{}, root, a *solutions.TreeNode, b *piscine.TreeNode) {
	challenge.Fatalf("%s(\n%s) == %s instead of %s\n",
		"BTreeMin",
...
func CompareNode_min(fn interface{}, arg1, a *solutions.TreeNode, b *piscine.TreeNode) {
	if a == nil || b == nil {
...
func CompareReturn_min(fn1, fn2, arg1, arg2 interface{}) {
	...
	for i, v := range out1.Results {
		...
		case *solutions.TreeNode:
			CompareNode_min(fn1, arg1.(*solutions.TreeNode), str, out2.Results[i].(*piscine.TreeNode))
		default:
...
func main() {
	...
	rootS := &piscine.TreeNode{Data: "04"}

	ins := []string{"03", "02", "01", "07", "05", "12", "10"}

	for _, v := range ins {
		root = solutions.BTreeInsertData(root, v)
		rootS = piscine.BTreeInsertData(rootS, v)
	}

	CompareReturn_min(solutions.BTreeMin, piscine.BTreeMin, root, rootS)
}
